...
#timelen = np.loadtxt(r'/home/vboxuser/pencil-kbobinary/gravitational-scattering/data/tstalk.dat')
#print(timelen)
#nstalk  = int(timelen[1]) # Second element of timelen provides number of pstalk timesteps recorded
#header  = np.loadtxt(r'/home/vboxuser/pencil-kbobinary/gravitational-scattering/output/file_0001.txt')
#print(header)
#header_len = len(header)  # Second element of .shape array provides number of columns in each pstalk timestep

## Create new array to input particle 1 data
#parray1 = np.empty(shape=(nstalk, header_len))
#for i in range(0, nstalk-1): 
#    num = '{0:04}'.format(i+1) # Format each output file call e.g. 1 -> '0001'
#    output = np.loadtxt(r'/home/vboxuser/pencil-kbobinary/gravitational-scattering/output/file_'+num+'.txt')
#    parray1[i,:] = output # First row elements are substituted into the larger array
#print(parray1)
