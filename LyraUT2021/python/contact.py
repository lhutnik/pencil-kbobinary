...
contact = np.zeros([nr,nb])

for ib in range(nb):
    contact[:,ib] = tau_eff * vbin[:,ib] / ugas_eff 
...
img = ax1.contourf(rsun/AU,binary_separation,np.log10(np.transpose(contact)/yr),256,cmap="inferno")
...
for l,s in zip( CS4.levels, strs ):
    fmt[l] = s
#ax1.clabel(CS4, fmt='%2.1f', colors='w', fontsize=14)
